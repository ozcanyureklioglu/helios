...
public class WarehouseInventoryService : IWarehouseInventoryService
{
    ...
    public async Task<ApiResponse<Helios.Application.Features.WarehouseInventories.Dtos.BulkCreateWarehouseInventoryResponse>> CreateWarehouseInventoryAsync(
        List<CreateWarehouseInventoryRequest> requests,
        CancellationToken cancellationToken = default)
    {
        var responseData = new Helios.Application.Features.WarehouseInventories.Dtos.BulkCreateWarehouseInventoryResponse();

        if (requests == null || !requests.Any())
            return ApiResponse<Helios.Application.Features.WarehouseInventories.Dtos.BulkCreateWarehouseInventoryResponse>.Fail("Liste boş olamaz.");
        ...
        foreach (var request in requests)
        {
            ...
            {
                responseData.Errors.Add(new Helios.Application.Features.WarehouseInventories.Dtos.WarehouseInventoryErrorDto
                {
        ...
        if (validInventories.Any())
        {
            ...
            var publishTasks = validInventories.Select(inventory => 
                _publishEndpoint.Publish(new Helios.Application.Common.Events.CreateWarehouseInventoryEvent
                {
        ...
        return ApiResponse<Helios.Application.Features.WarehouseInventories.Dtos.BulkCreateWarehouseInventoryResponse>.Ok(responseData, "İşlem tamamlandı.");
    }

    public async Task<ApiResponse<List<Helios.Application.Features.WarehouseInventories.Dtos.WarehouseInventoryDto>>> GetWarehouseInventoriesAsync(
        Guid? productId = null,
        CancellationToken cancellationToken = default)
    {
        ...
        var inventories = await query
            .Select(wi => new Helios.Application.Features.WarehouseInventories.Dtos.WarehouseInventoryDto(
                wi.Product.Name,
                wi.Product.Sku,
                wi.Warehouse.Name,
        ...
        return ApiResponse<List<Helios.Application.Features.WarehouseInventories.Dtos.WarehouseInventoryDto>>.Ok(inventories);
    }
}
